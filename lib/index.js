"use strict";
function parseConsoleStyles(log) {
    var text = log[0].split(/(?=%c)/g);
    var parameters = log.slice(1);
    var styles = parameters.filter(function (p) { return typeof p == 'string' && p.startsWith('color: '); });
    var params = parameters.filter(function (p) { return typeof p == 'string' && !p.startsWith('color: '); });
    var count = {
        styles: 0,
        params: 0
    };
    ...
    text.map(function (t, i) {
        if (t.startsWith('%c')) {
            ...
            output.push({ t: t, color: styles[count.styles].replace('color: ', '') });
            count.styles++;
        }
        else {
            if (t.includes('%s')) {
...
exports.parseConsoleStyles = parseConsoleStyles;
